...
export class DHCipherLayer implements CipherLayer {
  ...
  async encrypt(data: Uint8Array, keys: DHKeys): Promise<EncryptedPayload> {
    ...
    try {
      ...
      } else if (keys.privateKey && keys.publicKey) {
      ...
      const contextIdCopy = Array.from(contextId); // Convert to array for serialization
      ...
      return {
        ...
        layerMetadata: {
          ...
          timestamp, // Use the same timestamp as HKDF and AAD
        ...
        parameters: {
          iv: ivCopy,
          salt: saltCopy,
          contextId: contextIdCopy, // Store as array, not Uint8Array
        },
      };
    } catch (error) {
      throw new CipherLayerError(
