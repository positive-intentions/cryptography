...
const MultiProtocolDemo = () => {
  ...
  const [message, setMessage] = useState(
    "Secret message with MLS + Signal + ML-KEM + DH + AES!",
  );
  ...
  const initializeMLKEM = async () => {
    try {
      addLog(
        "🔐 Initializing ML-KEM (quantum-resistant) for Alice and Bob...",
        "info",
      );
      ...
      addLog(
        `📊 Alice public key size: ${aliceKeyPair.publicKey.key.length} bytes`,
        "info",
      );
      ...
      addLog(
        `📊 Bob public key size: ${bobKeyPair.publicKey.key.length} bytes`,
        "info",
      );
  ...
  const handleEncrypt = async () => {
    ...
    if (enableSignal && !signalInitialized)
      missingInitializations.push("Signal");
    ...
    try {
      ...
      addLog(
        `📈 Size increase: ${((result.finalSize / result.originalSize - 1) * 100).toFixed(1)}%`,
        "info",
      );
      addLog(
        `⏱️ Total encryption time: ${result.totalProcessingTime.toFixed(2)}ms`,
        "info",
      );
      ...
      for (let round = 0; round < rounds; round++) {
        const roundLayers = result.layers.slice(
          round * layersPerRound,
          (round + 1) * layersPerRound,
        );
        if (roundLayers.length > 0) {
          const roundTime = roundLayers.reduce(
  ...
  const handleDecrypt = async () => {
    ...
    try {
      ...
      if (plaintextStr === message) {
        addLog(
          "🎉 Round-trip successful! Messages match perfectly!",
          "success",
        );
      } else {
        addLog(
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Typography variant="body1" color="text.secondary" paragraph>
        Demonstrates customizable cascading encryption with MLS, Signal, DH, and
        AES layers. Toggle individual layers on/off and apply them multiple
        ...
        <Card>
          <CardContent>
            ...
            <Typography variant="body2" color="text.secondary" paragraph>
              Initialize MLS group messaging, Signal Protocol Double Ratchet,
            ...
            <Stack direction="row" spacing={2} sx={{ mb: 2 }}>
              <Chip
                label={
                  mlsInitialized ? "✅ MLS Ready" : "⏳ MLS Not Initialized"
                }
          ...
          <Card>
            <CardContent>
              ...
              <Accordion>
                <AccordionSummary>
                  <Typography>
                    📊 Metadata ({rounds} rounds, {encrypted.layers.length}{" "}
                    layers total)
                  </Typography>
                </AccordionSummary>
                <AccordionDetails>
                  ...
                  {encrypted.layers.map((layer, i) => (
                    ...
                    >
                      {i + 1}. {layer.algorithm}: {layer.inputSize}B →{" "}
                      {layer.outputSize}B ({layer.processingTime.toFixed(2)}ms)
                    </Typography>
                  ))}
                </AccordionDetails>
              </Accordion>
