...
export default {
  ...
  parameters: {
    docs: {
      description: {
        component: 'Demonstration of zeroization (memory clearing) for sensitive buffers in MLS and Signal cipher layers.',
      },
    },
  },
};
...
const ZeroizationDemoComponent = () => {
  ...
  const testMLSZeroization = async () => {
    ...
    try {
      ...
      const addResult = await senderManager.addMembers(groupId, [receiverKeyPackage]);
      await receiverManager.processWelcome(addResult.welcome, addResult.ratchetTree);
      ...
      setTestResults({
        mlsTest: '✅ MLS zeroization test passed - encryption/decryption works correctly with zeroization. Zeroization occurs in base64 conversion methods.',
        success: true,
        details: {
  ...
  const testSignalZeroization = async () => {
    setLoading(true);
    setTestResults({ signalTest: 'Testing Signal zeroization...', success: null });
    try {
      ...
      const envelope = await crypto.doubleRatchetEncrypt(aliceState, testMessage);
      ...
      <Stack spacing={3}>
        <Alert severity="info">
          <Typography variant="body2">
            <strong>Zeroization:</strong> Sensitive buffers are cleared from memory after use to prevent
            key material from lingering in RAM. This is especially important in JavaScript environments
              ...
              <Stack spacing={2}>
                ...
                {testResults.before && (
                  <Grid container spacing={2}>
                    <Grid item xs={12} sm={6}>
                      <Paper sx={{ p: 2 }}>
                        <Typography variant="caption" color="text.secondary">Before Zeroization</Typography>
                        <CodeDisplay
                    ...
                    <Grid item xs={12} sm={6}>
                      <Paper sx={{ p: 2 }}>
                        <Typography variant="caption" color="text.secondary">After Zeroization</Typography>
                        <CodeDisplay
                ...
                {testResults.success !== undefined && (
                  <OperationStatus
                    ...
                    message={testResults.success ? 'All bytes zeroized successfully' : 'Zeroization failed'}
                  />
                )}
              </Stack>
            </CardContent>
            ...
            <CardContent>
              <Stack spacing={2}>
                ...
                <Box component="ul" sx={{ pl: 2 }}>
                  <li><code>arrayBufferToBase64()</code> - zeroizes bytes buffer</li>
                  <li><code>base64ToArrayBuffer()</code> - zeroizes bytes buffer</li>
                  <li><code>encrypt()</code> - zeroizes base64Data buffer</li>
                  <li><code>decrypt()</code> - zeroizes base64Data buffer</li>
                </Box>
                <Button 
                  variant="contained" 
                ...
                {testResults.mlsTest && (
                  <>
                    <OperationStatus
                      status={testResults.success ? 'success' : testResults.success === false ? 'error' : 'info'}
          ...
          <Card>
            ...
            <CardContent>
              <Stack spacing={2}>
                ...
                <Box component="ul" sx={{ pl: 2 }}>
                  <li><code>webCryptoEncrypt()</code> - zeroizes key buffer</li>
                  <li><code>webCryptoDecrypt()</code> - zeroizes key buffer</li>
                  <li><code>encrypt()</code> - zeroizes messageKey, nonce, aad buffers</li>
                  <li><code>decrypt()</code> - zeroizes messageKey, nonce, aad buffers</li>
                </Box>
                <Button 
                  variant="contained" 
        ...
        <Card>
          ...
          <CardContent>
            <Stack spacing={1}>
              <Chip label="Prevents key material leakage" color="success" size="small" />
              <Chip label="Reduces memory exposure window" color="success" size="small" />
              <Chip label="Exception-safe cleanup" color="success" size="small" />
              <Chip label="Follows security best practices" color="success" size="small" />
            </Stack>
          </CardContent>
        </Card>
      </Stack>
