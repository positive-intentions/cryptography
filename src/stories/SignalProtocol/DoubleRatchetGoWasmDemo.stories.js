...
const DoubleRatchetGoWasmDemo = () => {
  ...
  const generateKeys = async () => {
    ...
    try {
      ...
      const aliceIdentity =
        await signalProtocolRef.current.generateIdentityKeyPair();
      const aliceRegistrationId =
        signalProtocolRef.current.generateRegistrationId();
      ...
      setMessages([
        {
          ...
          sender: "system",
          content:
            "🔐 Secure session established using Double Ratchet protocol",
  ...
  const sendMessage = async () => {
    if (
      !signalProtocolRef.current ||
      !inputMessage.trim() ||
      !demoState.sessionEstablished
    )
      return;
  ...
  const steps = [
    {
      ...
      description:
        "Generate identity key pairs, registration IDs, and pre-keys for both Alice and Bob",
      icon: <KeyIcon />,
      action: generateKeys,
    ...
    },
    {
      ...
      description:
        "Exchange pre-key bundles and establish Double Ratchet session",
      icon: <LockIcon />,
      ...
      <Box sx={{ mb: 3 }}>
        <Alert severity="info" sx={{ mb: 3 }}>
          <Typography variant="body2">
            <strong>Production-Ready Go Implementation</strong>
            <br />
            This demo uses the audited libsignal-protocol-go library compiled to
            WebAssembly. The Double Ratchet protocol provides perfect forward
        ...
        <Paper
          sx={{
            p: 2,
            mb: 3,
            bgcolor: wasmReady ? "success.light" : "warning.light",
          }}
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
            ...
            <Typography variant="h6">
              Go WASM Status:{" "}
              {loading ? "Loading..." : wasmReady ? "Ready" : "Not Available"}
            </Typography>
            {wasmReady && (
              <Chip
        ...
        <OperationStatus
          loading={loading}
          error={error}
          success={demoState.sessionEstablished}
        />
              ...
              <Grid item xs={12} md={4} key={index}>
                <Card
                  sx={{
                    ...
                    borderColor:
                      currentStep === index ? "primary.main" : "divider",
                  ...
                  <Card>
                    <CardContent>
                      ...
                      <Typography variant="body2" sx={{ mb: 1, mt: 1 }}>
                        <strong>Pre-keys Generated:</strong>{" "}
                        {alice.preKeys.length}
                      </Typography>
                      <Typography variant="body2">
                        <strong>Signed Pre-key ID:</strong>{" "}
            ...
            <Card>
              <CardContent>
                ...
                <Typography variant="body2" sx={{ mt: 2 }}>
                  Messages exchanged: {demoState.messagesExchanged} | Each
                  message uses a new encryption key (forward secrecy)
                </Typography>
              </CardContent>
            </Card>
          </Box>
          ...
          <Box>
            <Accordion defaultExpanded>
              <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Typography variant="h6">
                  Double Ratchet Protocol Explanation
                </Typography>
              </AccordionSummary>
              <AccordionDetails>
                <Typography variant="body2" paragraph>
                  The Double Ratchet algorithm is used by Signal Protocol to
                  provide:
                </Typography>
                <List>
                  <ListItem>
                ...
                <Typography variant="body2" component="div">
                  <ol>
                    <li>
                      <strong>Diffie-Hellman Ratchet:</strong> Updates shared
                      secrets with new key exchanges
                    </li>
                    <li>
                      <strong>Symmetric-key Ratchet:</strong> Derives message
                      keys from chain keys
                    </li>
                    <li>
                      <strong>Chain Keys:</strong> Updated after each message
                      for forward secrecy
                    </li>
                    <li>
                      <strong>Message Keys:</strong> Deleted immediately after
                      use
                    </li>
                  </ol>
                </Typography>
            ...
            <Accordion>
              ...
              <AccordionDetails>
                <Typography variant="body2" paragraph>
                  Extended Triple Diffie-Hellman (X3DH) establishes the initial
                  shared secret:
                </Typography>
                <Typography variant="body2" component="div">
                  <ol>
                    <li>
                      <strong>Identity Keys:</strong> Long-term keys for
                      authentication
                    </li>
                    <li>
                      <strong>Signed Pre-keys:</strong> Medium-term keys signed
                      by identity key
                    </li>
                    <li>
                      <strong>One-time Pre-keys:</strong> Ephemeral keys for
                      added security
                    </li>
                    <li>
                      <strong>Ephemeral Keys:</strong> Session-specific keys
                    </li>
                  </ol>
                </Typography>
                <Alert severity="info" sx={{ mt: 2 }}>
                  <Typography variant="body2">
                    The Go implementation uses Curve25519 for all elliptic curve
                    operations, providing 128-bit security level.
                  </Typography>
                </Alert>
              </AccordionDetails>
            </Accordion>
                    ...
                    {performance.encryption.length > 0 && (
                      <Box sx={{ mb: 2 }}>
                        ...
                        <LinearProgress
                          ...
                          value={Math.min(
                            100,
                            getAverageTime(performance.encryption) / 5,
                          )}
                        />
                        <Typography variant="caption">
                    ...
                    {performance.decryption.length > 0 && (
                      <Box>
                        ...
                        <LinearProgress
                          ...
                          value={Math.min(
                            100,
                            getAverageTime(performance.decryption) / 5,
                          )}
                        />
                        <Typography variant="caption">
                ...
                <Card>
                  <CardContent>
                    ...
                    <List dense>
                      ...
                      <ListItem>
                        <ListItemText primary="Curve" secondary="Curve25519" />
                      </ListItem>
                      <ListItem>
                        <ListItemText
                          primary="Signatures"
                      ...
                      <ListItem>
                        <ListItemText primary="KDF" secondary="HKDF-SHA256" />
                      </ListItem>
                      <ListItem>
                        <ListItemText primary="WASM Size" secondary="~8.4 MB" />
                      </ListItem>
                    </List>
                  </CardContent>
                </Card>
