...
describe("MLS Commit Synchronization", () => {
  ...
  test("Scenario 1: Raw commit object (no encoding)", async () => {
    ...
    console.log(
      "Approach: Pass commitResult.commit directly to processPublicMessage",
    );
  ...
  test("Scenario 2: Encoded commit bytes", async () => {
    ...
    console.log(
      'Current issue: Cannot read properties of undefined (reading "content")',
    );
  });
...
window.debugMLSCommit = async (commitResult) => {
  ...
  console.log(
    "commit keys:",
    commitResult.commit ? Object.keys(commitResult.commit) : "N/A",
  );
