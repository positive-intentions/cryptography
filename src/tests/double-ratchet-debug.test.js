...
describe('Double Ratchet AAD Debug Test', () => {
    ...
    test('should identify AAD construction mismatch', async () => {
        ...
        console.log('  DH Public Key:', Array.from(testDHPublicKey).map(b => b.toString(16).padStart(2, '0')).join(''));
        ...
        const encryptionView = new DataView(encryptionAAD.buffer, testDHPublicKey.length);
        ...
        const decryptionView_original = new DataView(decryptionAAD_original.buffer, testDHPublicKey.length);
        decryptionView_original.setUint32(0, messageNumber, true);
        decryptionView_original.setUint32(4, previousChainLength, true);
        
        console.log('🔓 Decryption AAD (original/buggy):', Array.from(decryptionAAD_original));
        ...
        const decryptionView_fixed = new DataView(decryptionAAD_fixed.buffer, testDHPublicKey.length);
        ...
        const encryptionEquals_original = encryptionAAD.every((byte, i) => byte === decryptionAAD_original[i]);
        const encryptionEquals_fixed = encryptionAAD.every((byte, i) => byte === decryptionAAD_fixed[i]);
        
        console.log('🔍 AAD Comparisons:');
        console.log('  Encryption vs Original Decryption:', encryptionEquals_original);
    ...
    test('should demonstrate working AES-GCM with correct AAD', async () => {
        ...
        const decryptionAAD = new Uint8Array(messageEnvelope.dhPublicKey.length + 8);
        decryptionAAD.set(messageEnvelope.dhPublicKey); // Fixed: direct assignment
        const decryptionView = new DataView(decryptionAAD.buffer, messageEnvelope.dhPublicKey.length);
        ...
        const decryptedPlaintext = await crypto.subtle.decrypt(
            { name: "AES-GCM", iv: messageEnvelope.iv, additionalData: decryptionAAD },
            testKey,
