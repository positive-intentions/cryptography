        ...
        this.importObject = {
            go: {
                ...
                'runtime.getRandomData': (sp) => {
                    ...
                    const arr = new Uint8Array(this._inst.exports.mem.buffer, Number(slice), 8);
...
describe('libsignal-protocol-go WASM Implementation', () => {
    ...
    beforeAll(async () => {
        try {
            ...
            const wasmPath = path.join(process.cwd(), '..', 'libsignal-protocol-go', 'wasm', 'signal.wasm');
    ...
    describe('Key Generation', () => {
        ...
        test('generates signed pre-key', async () => {
            ...
            const signedPreKey = await SignalProtocol.generateSignedPreKey('mockPrivKey', 'mockPubKey', 1);
    ...
    describe('Message Encryption/Decryption', () => {
        ...
        test('encrypts a message', async () => {
            ...
            const encrypted = await SignalProtocol.encryptMessage(aliceId, bobId, message);
        ...
        test('full encryption/decryption flow', async () => {
            ...
            await SignalProtocol.processPreKeyBundle(aliceId, bobId, JSON.stringify(bundle));
            
            // Encrypt message from Alice to Bob
            const originalMessage = 'Secret message from Alice';
            const encrypted = await SignalProtocol.encryptMessage(aliceId, bobId, originalMessage);
            
            expect(encrypted.ciphertext).toBeDefined();
            
            // Decrypt message at Bob's side
            const decrypted = await SignalProtocol.decryptMessage(bobId, aliceId, encrypted.ciphertext);
    ...
    describe('Double Ratchet Protocol', () => {
        test('maintains forward secrecy', async () => {
            ...
            for (const msg of messages) {
                const encrypted = await SignalProtocol.encryptMessage(aliceId, bobId, msg);
                const decrypted = await SignalProtocol.decryptMessage(bobId, aliceId, encrypted.ciphertext);
                
                expect(decrypted).toBe(msg);
                
                // Each encryption should produce different ciphertext (forward secrecy)
                const encrypted2 = await SignalProtocol.encryptMessage(aliceId, bobId, msg);
    ...
    describe('Error Handling', () => {
        ...
        test('handles invalid ciphertext gracefully', async () => {
            ...
            await expect(async () => {
                await SignalProtocol.decryptMessage(bobId, aliceId, 'invalid_ciphertext');
            }).rejects.toThrow();
        });
    });
    ...
    describe('Performance', () => {
        ...
        test('encryption/decryption performance', async () => {
            ...
            for (let i = 0; i < iterations; i++) {
                const encrypted = await SignalProtocol.encryptMessage(aliceId, bobId, message);
                await SignalProtocol.decryptMessage(bobId, aliceId, encrypted.ciphertext);
            }
    ...
    describe('Compatibility', () => {
        ...
        test('handles large messages', async () => {
            ...
            const encrypted = await SignalProtocol.encryptMessage(aliceId, bobId, largeMessage);
            const decrypted = await SignalProtocol.decryptMessage(bobId, aliceId, encrypted.ciphertext);
