...
describe('WASM Double Ratchet Debug Tests', () => {
    ...
    beforeAll(async () => {
        try {
            ...
            if (typeof jest !== 'undefined') {
                ...
                wasmModule = {
                    initialize_double_ratchet: jest.fn().mockImplementation((sharedSecret, isInitiator) => {
                    ...
                    double_ratchet_decrypt: jest.fn().mockImplementation((state, encryptedMessage) => {
            ...
            console.log('✅ Shared secret created:', Array.from(sharedSecret.slice(0, 8)).map(b => b.toString(16)).join(''));
        } catch (error) {
    ...
    test('should initialize Alice and Bob states correctly', () => {
        ...
        console.log('  - Sending message number:', aliceState.sending_message_number);
        console.log('  - Receiving message number:', aliceState.receiving_message_number);
        ...
        console.log('  - Receiving message number:', bobState.receiving_message_number);
    ...
    test('should handle Alice sending first message to Bob', () => {
        ...
        console.log('  - DH public key length:', encryptedMessage.dh_public_key.length);
        console.log('  - Message number:', encryptedMessage.message_number);
        console.log('  - Previous chain length:', encryptedMessage.previous_chain_length);
    ...
    test('should handle bidirectional conversation', () => {
        ...
        try {
            ...
            console.log('  - Previous chain length:', encrypted2.previous_chain_length);
            ...
            const decrypted2 = wasmModule.double_ratchet_decrypt(aliceState, encrypted2);
    ...
    test('should debug DH ratchet step process', () => {
        ...
        try {
            ...
            console.error('❌ This indicates the core issue with WASM implementation');
            
            // Detailed error analysis
            if (error.message.includes('No receiving chain key')) {
                console.error('🔍 ROOT CAUSE: Receiving chain key not properly established during DH ratchet');
            } else if (error.message.includes('No sending chain key')) {
                console.error('🔍 ROOT CAUSE: Sending chain key not properly established during DH ratchet');
            }
