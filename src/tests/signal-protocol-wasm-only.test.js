...
describe('Signal Protocol WASM Implementation (WASM Only)', () => {
    ...
    beforeAll(async () => {
        ...
        if (!fs.existsSync(wasmFile) || !fs.existsSync(jsFile)) {
            console.warn('⚠️ WASM files not found. Run "npm run build:wasm" to build WASM first.');
            wasmAvailable = false;
            return;
        }
    ...
    test('WASM availability check', () => {
        if (wasmAvailable) {
            ...
            console.log('⚠️ WASM not available - tests skipped. Run "npm run build:wasm" first.');
