...
describe('Double Ratchet WASM Flow Reproduction', () => {
        ...
        try {
            ...
            if (typeof jest !== 'undefined') {
                ...
                wasmModule = {
                    ...
                    double_ratchet_decrypt: jest.fn().mockReturnValue('decrypted-message')
                };
                return;
            }
            
            // Load the real WASM module from Frontend/pkg
            const SignalWasm = await import('../../Frontend/pkg/signal_protocol_wasm.js');
    ...
    test('should reproduce the message flow logic step by step', async () => {
        ...
        const aliceState = wasmModule.initialize_double_ratchet(sharedSecretMock, true);
        
        console.log('5. ✅ Bob WASM state initialized (responder)');
        const bobState = wasmModule.initialize_double_ratchet(sharedSecretMock, false);
        
        console.log('6. ✅ Alice encrypts message via WASM');
        const message = 'Hello Bob from Alice!';
        const encryptedMessage = wasmModule.double_ratchet_encrypt(aliceState, message);
        ...
        const decryptedMessage = wasmModule.double_ratchet_decrypt(bobState, encryptedMessage);
        
        if (typeof jest !== 'undefined') {
            // In Jest environment with mocks, verify the functions were called
            expect(wasmModule.initialize_double_ratchet).toHaveBeenCalledWith(sharedSecretMock, true);
            expect(wasmModule.initialize_double_ratchet).toHaveBeenCalledWith(sharedSecretMock, false);
            expect(wasmModule.double_ratchet_encrypt).toHaveBeenCalledWith(aliceState, message);
            expect(wasmModule.double_ratchet_decrypt).toHaveBeenCalledWith(bobState, encryptedMessage);
    ...
    test('should identify the specific AES-GCM issue', async () => {
        ...
        console.log('- Bob DH ratchet: SUCCESS (initial receiving chain established)');
        console.log('- Bob message key derivation: UNKNOWN (likely where issue is)');
        console.log('- Bob AES-GCM decryption: FAILED (aead::Error)');
        
        console.log('\n🎯 Most Likely Issues:');
        console.log('1. Message key derivation differs between Alice encrypt & Bob decrypt');
    ...
    test('should examine the chain key advancement issue', async () => {
        ...
        console.log('\n💡 Potential Issues:');
        console.log('1. Alice and Bob using different chain key derivation methods');
        ...
        console.log('1. Ensure Bob\'s initial receiving chain = Alice\'s sending chain');
    ...
    test('should identify the root cause - chain key synchronization', async () => {
        ...
        console.log('  - Derives sending chain from root key with "Signal_Initial_Chain"');
        console.log('  - Uses HKDF(root_key, "Signal_Initial_Chain", "chain_key")');
        
        console.log('\nBob (responder on first message):');
        console.log('  - Derives receiving chain from root key with "Signal_Initial_Receive"');
        console.log('  - Uses HKDF(root_key, "Signal_Initial_Receive", "receiving_chain")');
        
        console.log('\n❌ PROBLEM: Different HKDF parameters = Different chain keys!');
        console.log('Alice and Bob end up with different chain keys, so:');
        console.log('- Alice encrypts with message_key_A = HMAC(chain_key_A, 0x01)');
        console.log('- Bob decrypts with message_key_B = HMAC(chain_key_B, 0x01)');
        console.log('- message_key_A ≠ message_key_B → AES-GCM authentication fails');
        
        console.log('\n✅ CORRECT FIX:');
        console.log('Bob\'s initial receiving chain key must equal Alice\'s sending chain key');
        console.log('Both should use the same HKDF derivation from the shared root key');
