...
describe("KeyAuthentication Input Validation", () => {
  ...
  describe("generateFingerprint Input Validation", () => {
    test("should reject null public key", async () => {
      ...
      await expect(
        KeyAuthentication.generateFingerprint(null),
      ).rejects.toThrow();
    });
    ...
    test("should reject undefined public key", async () => {
      ...
      await expect(
        KeyAuthentication.generateFingerprint(undefined),
      ).rejects.toThrow();
    });
    ...
    test("should reject empty Uint8Array", async () => {
      ...
      await expect(
        KeyAuthentication.generateFingerprint(emptyKey),
      ).rejects.toThrow();
    });
    ...
    test("should reject Uint8Array that is too large", async () => {
      ...
      largeKey.fill(1);
      await expect(
        KeyAuthentication.generateFingerprint(largeKey),
      ).rejects.toThrow();
    });
    ...
    test("should accept Uint8Array at maximum size", async () => {
      ...
      const fingerprint =
        await KeyAuthentication.generateFingerprint(maxSizeKey);
      expect(fingerprint).toBeDefined();
      expect(typeof fingerprint).toBe("string");
    });
    ...
    test("should handle CryptoKey with export restrictions", async () => {
      ...
      try {
        const fingerprint =
          await KeyAuthentication.generateFingerprint(hmacKey);
    ...
    test("should reject non-key objects", async () => {
      ...
      await expect(
        KeyAuthentication.generateFingerprint(invalidObject),
      ).rejects.toThrow();
    });
    ...
    test("should reject string inputs", async () => {
      ...
      await expect(
        KeyAuthentication.generateFingerprint("not-a-key"),
      ).rejects.toThrow();
    });
    ...
    test("should reject number inputs", async () => {
      ...
      await expect(
        KeyAuthentication.generateFingerprint(12345),
      ).rejects.toThrow();
    });
    ...
    test("should reject array inputs", async () => {
      ...
      await expect(
        KeyAuthentication.generateFingerprint([1, 2, 3, 4, 5]),
      ).rejects.toThrow();
    });
  });
  ...
  describe("verifyFingerprint Input Validation", () => {
    ...
    test("should reject invalid fingerprint format", async () => {
      ...
      const result = await KeyAuthentication.verifyFingerprint(
        keyBytes,
        invalidFingerprint,
      );
  ...
  describe("Error Message Security", () => {
    test("should not leak key data in error messages", async () => {
      ...
      const keyHex = Array.from(sensitiveKey)
        .map((b) => b.toString(16))
        .join("");
