...
describe('Signal Protocol Implementation', () => {
    ...
    describe('Message Encryption and Key Rotation', () => {
        ...
        beforeEach(() => {
            ...
            mockCrypto.subtle.deriveKey.mockImplementation((params, baseKey, derivedKeyType) => {
                // Create different keys based on message number in the info field
                const info = new TextDecoder().decode(params.info);
                const messageNum = info.match(/message_(\d+)|secure_message_(\d+)/)?.[1] || info.match(/secure_message_(\d+)/)?.[2] || '0';
            ...
            mockCrypto.subtle.decrypt.mockImplementation((algorithm, key, ciphertext) => {
                const ciphertextStr = new TextDecoder().decode(ciphertext);
                
                // Extract message info from ciphertext to detect replays
                const messageMatch = ciphertextStr.match(/^encrypted_(.+?)_with_key_(.+)$/);
                if (messageMatch) {
                    ...
                    if (processedMessages.has(messageId) && originalKeyId !== key.keyId) {
        ...
        test('should encrypt and decrypt messages with different keys for each message', async () => {
            ...
            for (let i = 0; i < messages.length; i++) {
                const envelope = await encryptMessage(mockSharedSecret, messages[i], i + 1);
            ...
            expect(mockCrypto.subtle.deriveKey).toHaveBeenCalledTimes(messages.length * 2); // Once for encrypt, once for decrypt
        ...
        test('should handle bidirectional conversation with proper key rotation', async () => {
            ...
            const sendMessage = async (sender, message) => {
                ...
                // Derive message key
                const prk = await crypto.subtle.importKey("raw", mockSharedSecret, "HKDF", false, ["deriveKey"]);
                const messageKey = await crypto.subtle.deriveKey(
                    {
                        name: "HKDF",
                        hash: "SHA-256", 
            ...
            const conversationFlow = [
                ...
                { sender: 'Bob', message: 'Perfect! Each message should use different keys.' }
            ];
        ...
        test('should prevent message replay attacks', async () => {
            ...
            await expect(decryptMessage(replayedMsg1, 3)).rejects.toThrow('Message replay detected');
        });
    });
    ...
    describe('Complete Signal Protocol Integration Tests', () => {
        ...
        beforeEach(async () => {
            ...
            mockCrypto.subtle.exportKey.mockImplementation((format, key) => {
                return Promise.resolve(new TextEncoder().encode(`exported_${key.keyId}`).buffer);
            });

            mockCrypto.subtle.sign.mockImplementation((algorithm, key, data) => {
                const dataStr = new TextDecoder().decode(data);
                return Promise.resolve(new TextEncoder().encode(`signature_${key.keyId}_${dataStr}`).buffer);
            });

            mockCrypto.subtle.verify.mockResolvedValue(true);

            mockCrypto.subtle.deriveBits.mockImplementation((algorithm, privateKey, length) => {
            ...
            sharedSecret = new TextEncoder().encode('alice_bob_shared_secret_12345').buffer;
        });
        ...
        test('should complete full X3DH key exchange with 4-DH', async () => {
            const performX3DH = async (alice, bobPublicBundle) => {
                ...
                const bobIdentityPublic = await crypto.subtle.importKey(
                    "raw", bobPublicBundle.identityKey,
                    { name: "ECDH", namedCurve: "P-256" }, false, []
                );

                const bobSignedPrekey = await crypto.subtle.importKey(
                    "raw", bobPublicBundle.signedPrekey,
                    { name: "ECDH", namedCurve: "P-256" }, false, []
                );

                const bobOneTimePrekey = bobPublicBundle.oneTimePrekey ? await crypto.subtle.importKey(
                    "raw", bobPublicBundle.oneTimePrekey,
                    { name: "ECDH", namedCurve: "P-256" }, false, []
                ) : null;
                ...
                const dh4 = bobOneTimePrekey ? await crypto.subtle.deriveBits(
                ...
                return {
                    ...
                    aliceEphemeralPublic: await crypto.subtle.exportKey("raw", aliceEphemeral.publicKey),
            ...
            const bobBundle = {
                identityKey: await crypto.subtle.exportKey("raw", bob.identityKeyPair.publicKey),
                signedPrekey: await crypto.subtle.exportKey("raw", bob.signedPrekeyPair.publicKey),
                oneTimePrekey: await crypto.subtle.exportKey("raw", bob.oneTimePrekeys[0].publicKey),
        ...
        test('should handle long conversation with 50 messages and proper key rotation', async () => {
            ...
            mockCrypto.subtle.encrypt.mockImplementation((algorithm, key, data) => {
                const plaintext = new TextDecoder().decode(data);
                return Promise.resolve(
                    new TextEncoder().encode(`encrypted_${plaintext}_key_${key.keyId}`).buffer
                );
            });
            ...
            mockCrypto.subtle.decrypt.mockImplementation((algorithm, key, ciphertext) => {
                ...
                if (ciphertextStr.includes(`_key_${key.keyId}`)) {
                    const plaintext = ciphertextStr.replace(`encrypted_`, '').replace(`_key_${key.keyId}`, '');
                    return Promise.resolve(new TextEncoder().encode(plaintext).buffer);
                }
            ...
            console.log(`✅ Successfully tested ${messageCount}-message conversation with perfect key rotation`);
        });
        ...
        test('should maintain security properties across conversation', async () => {
            const messages = [
                ...
                { sender: 'Alice', text: 'Absolutely! How about that forward secrecy?' },
    ...
    describe('Error Handling', () => {
        ...
        test('should handle one-time prekey exhaustion', async () => {
            ...
            expect(() => prekeyManager.consumePrekey())
                .toThrow('No one-time prekeys available');
        });
    });
});