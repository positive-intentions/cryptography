...
describe('AESCipherLayer Security', () => {
  ...
  describe('Scrypt Key Derivation', () => {
    ...
    test('should produce different keys with different passwords', async () => {
      ...
      const encrypted1 = await layer.encrypt(plaintext, { password: 'password1' });
      const encrypted2 = await layer.encrypt(plaintext, { password: 'password2' });

      // Ciphertexts should be different
      const cipher1Hex = Array.from(encrypted1.ciphertext).map(b => b.toString(16).padStart(2, '0')).join('');
      const cipher2Hex = Array.from(encrypted2.ciphertext).map(b => b.toString(16).padStart(2, '0')).join('');
    ...
    test('should produce different keys with different salts', async () => {
      ...
      const salt1Hex = Array.from(encrypted1.parameters.salt).map(b => b.toString(16)).join('');
      const salt2Hex = Array.from(encrypted2.parameters.salt).map(b => b.toString(16)).join('');
  ...
  describe('IV Reuse Protection', () => {
    test('should not reuse same IV for same key', async () => {
      ...
      const iv1Hex = Array.from(encrypted1.parameters.iv).map(b => b.toString(16).padStart(2, '0')).join('');
      const iv2Hex = Array.from(encrypted2.parameters.iv).map(b => b.toString(16).padStart(2, '0')).join('');
      const iv3Hex = Array.from(encrypted3.parameters.iv).map(b => b.toString(16).padStart(2, '0')).join('');
    ...
    test('should track IVs per password+salt combination', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const ivHex = Array.from(encrypted.parameters.iv).map(b => b.toString(16).padStart(2, '0')).join('');
