...
describe('MLS-AES Cascade', () => {
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      const cascadeModule = await import('../../crypto/CascadingCipher/CascadingCipherManager.ts');
  ...
  describe('MLS → AES Cascade', () => {
    test('should encrypt and decrypt MLS → AES cascade', async () => {
      if (!MLSCipherLayer || !MLSManager || !AESCipherLayer || !CascadingCipherManager) return;
    ...
    test('should trace data format at each step', async () => {
      ...
      console.log('MLS ciphertext (first 50 chars):', mlsCiphertextStr.substring(0, 50));
      ...
      console.log('AES decrypted matches MLS ciphertext:', 
      ...
      try {
        ...
        console.log('AES decrypted as UTF-8 (first 50 chars):', aesDecryptedStr.substring(0, 50));
      } catch (e) {
      ...
      const mlsDecrypted = await mlsLayer.decrypt(
        { ciphertext: aesDecrypted, layerMetadata: mlsEncrypted.layerMetadata, parameters: mlsEncrypted.parameters },
        mlsKeys
      );
