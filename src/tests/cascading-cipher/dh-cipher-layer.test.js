...
describe('DHCipherLayer', () => {
  ...
  beforeEach(async () => {
    ...
    // Dynamic import
    try {
      const module = await import('../../crypto/CascadingCipher/layers/DHCipherLayer.ts');
  ...
  describe('Key Validation', () => {
    ...
    test('should reject keys without required fields', () => {
      ...
      // Only privateKey
      expect(layer.validateKeys({ privateKey: new Uint8Array(32) })).toBe(false);
  ...
  describe('Decryption', () => {
    ...
    test('should fail with wrong shared secret', async () => {
      ...
      await expect(layer.decrypt(encrypted, { sharedSecret: wrongSecret })).rejects.toThrow();
    });
    ...
    test('should fail with corrupted ciphertext', async () => {
      ...
      await expect(layer.decrypt(encrypted, { sharedSecret })).rejects.toThrow();
    });
  ...
  describe('Error Handling', () => {
    test('should throw error with invalid keys on encrypt', async () => {
      ...
      await expect(layer.encrypt(plaintext, { privateKey: new Uint8Array(32) })).rejects.toThrow();
    });
