...
describe('Cascading Cipher Integration', () => {
  ...
  beforeEach(async () => {
    ...
    try {
      const managerModule = await import('../../crypto/CascadingCipher/CascadingCipherManager.ts');
  ...
  describe('Round-Robin Scenarios', () => {
    test('should apply same layer sequence multiple times', async () => {
      ...
      for (let round = 0; round < 3; round++) {
        const aes1 = new AESCipherLayer();
        const aes2 = new AESCipherLayer();
        Object.defineProperty(aes1, 'name', { value: `AES-Round${round}-Layer1` });
        Object.defineProperty(aes2, 'name', { value: `AES-Round${round}-Layer2` });
        manager.addLayer(aes1);
        manager.addLayer(aes2);
      }
      ...
      const keys = {};
      for (let round = 0; round < 3; round++) {
        keys[`AES-Round${round}-Layer1`] = { password: `password-r${round}-l1` };
        keys[`AES-Round${round}-Layer2`] = { password: `password-r${round}-l2` };
      }
    ...
    test('should apply DH + AES sequence multiple times', async () => {
      ...
      const keyPairs = [];
      for (let i = 0; i < 4; i++) {
        keyPairs.push(await crypto.subtle.generateKey(
          { name: 'ECDH', namedCurve: 'P-256' },
          true,
          ['deriveKey', 'deriveBits']
        ));
