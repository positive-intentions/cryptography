...
describe('SignalCipherLayer', () => {
  ...
  beforeEach(async () => {
    ...
    mockWasmModule = {
      ...
      DoubleRatchetMessage: {
        new: jest.fn((publicKey, messageNumber, previousChainLength, ciphertext) => {
          return {
            publicKey,
            messageNumber,
            previousChainLength,
      ...
      double_ratchet_decrypt: jest.fn((state, message) => {
        ...
        const plaintext = message.ciphertext.slice(16, message.ciphertext.length - 16);
    ...
    try {
      const module = await import('../../crypto/CascadingCipher/layers/SignalCipherLayer.ts');
  ...
  describe('Initialization', () => {
    ...
    test('should initialize with only wasmModule', async () => {
      ...
      expect(layer.validateKeys({ doubleRatchetState: mockDoubleRatchetState })).toBe(true);
    });
  });
  ...
  describe('Decryption', () => {
    test('should decrypt Signal-encrypted data', async () => {
      ...
      const decrypted = await layer.decrypt(encrypted, { doubleRatchetState: mockDoubleRatchetState });
    ...
    test('should reconstruct Double Ratchet message correctly', async () => {
      ...
      const encrypted = await layer.encrypt(plaintext, { doubleRatchetState: mockDoubleRatchetState });
      await layer.decrypt(encrypted, { doubleRatchetState: mockDoubleRatchetState });
  ...
  describe('State Management', () => {
    test('should handle multiple messages with incrementing message numbers', async () => {
      ...
      const msg1 = await layer.encrypt(
        new TextEncoder().encode('Message 1'),
        { doubleRatchetState: mockDoubleRatchetState }
      );
      const msg2 = await layer.encrypt(
        new TextEncoder().encode('Message 2'),
        { doubleRatchetState: mockDoubleRatchetState }
      );
      const msg3 = await layer.encrypt(
        new TextEncoder().encode('Message 3'),
        { doubleRatchetState: mockDoubleRatchetState }
      );

      // Message numbers should increment
      expect(msg2.parameters.messageNumber).toBeGreaterThan(msg1.parameters.messageNumber);
      expect(msg3.parameters.messageNumber).toBeGreaterThan(msg2.parameters.messageNumber);
    });
  ...
  describe('Integration with CascadingCipherManager', () => {
    test('should work as a layer in cascading cipher', async () => {
      ...
      const signalLayer = new SignalCipherLayer(mockWasmModule, mockDoubleRatchetState);
  ...
  describe('Error Handling', () => {
    ...
    test('should handle WASM decryption errors gracefully', async () => {
      ...
      const errorLayer = new SignalCipherLayer(errorWasm, mockDoubleRatchetState);

      await expect(
        errorLayer.decrypt(encrypted, { doubleRatchetState: mockDoubleRatchetState })
