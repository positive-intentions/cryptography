...
describe('WASM Module Structure and Build Validation', () => {
    ...
    test('should create expected WASM build artifacts', () => {
        ...
        expect(fs.existsSync(path.join(pkgDir, 'signal_protocol_wasm.js'))).toBe(true);
        expect(fs.existsSync(path.join(pkgDir, 'signal_protocol_wasm_bg.wasm'))).toBe(true);
        expect(fs.existsSync(path.join(pkgDir, 'signal_protocol_wasm.d.ts'))).toBe(true);
    ...
    test('should have correct TypeScript definitions', () => {
        const dtsFile = path.join(process.cwd(), 'pkg', 'signal_protocol_wasm.d.ts');
    ...
    test('should validate WASM file is not empty and has correct magic bytes', () => {
        const wasmFile = path.join(process.cwd(), 'pkg', 'signal_protocol_wasm_bg.wasm');
    ...
    describe('Modular Architecture Validation', () => {
        test('should contain documentation comments from modular structure', () => {
            const jsFile = path.join(process.cwd(), 'pkg', 'signal_protocol_wasm.js');
            const jsContent = fs.readFileSync(jsFile, 'utf8');
            
            // Check that documentation from our modular structure is included
            expect(jsContent).toContain('Generate an identity key pair');
            expect(jsContent).toContain('X3DH key agreement');
            expect(jsContent).toContain('message encryption');
        });
