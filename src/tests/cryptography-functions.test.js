...
import {
  ...
  useCryptography,
  randomString,
} from "../stories/components/Cryptography.tsx";
...
describe("Cryptography Functions - Target 100% Coverage", () => {
  ...
  test("Error handling paths", async () => {
    ...
    expect(console.error).toHaveBeenCalledWith(
      "Error deserializing private key:",
      expect.any(Error),
    );
    ...
    expect(console.error).toHaveBeenCalledWith(
      "Error deserializing symmetric key:",
      expect.any(Error),
    );
  ...
  test("File operations coverage", async () => {
    ...
    const binFile = new global.File([new Uint8Array([1, 2, 3])], "bin.bin", {
  ...
  test("Parse and decrypt uploaded files", async () => {
    ...
    const textUploadResult = await cryptoMethods.decryptUploadedFile(
      textFile,
      "password",
    );
    ...
    const mimeUploadResult = await cryptoMethods.decryptUploadedFile(
      mimeFile,
      "password",
    );
  ...
  test("File operation errors", async () => {
    ...
    expect(console.error).toHaveBeenCalledWith(
      "Error decrypting uploaded file:",
      expect.any(Error),
    );
  });
